...
describe('website smoke test', () => {
  ...
  it('renders the about page on the composed text page view', async () => {
    ...
    expect(host.textContent).toContain("Ceramics")
  ...
  it('colours and names the source-database chip from the manifest projects section', async () => {
    const { app, host } = await mountSite('#/item/1d61ec66-2c37-507e-9e98-65e166b78f82')
  ...
  it('shows the explore-partner notice only for the project dataset.config.js lists', async () => {
    const epm = await mountSite('#/item/94f76ace-8af6-5ec8-aec6-d15a08b2f250')
    ...
    const isl = await mountSite('#/item/1d61ec66-2c37-507e-9e98-65e166b78f82')
  ...
  it('renders the related-database and artistic-introduction links from the manifest', async () => {
    const [items] = await loadEntities(['items'])
    const item = items.find((i) => i.id === '1d61ec66-2c37-507e-9e98-65e166b78f82')
  ...
  it('renders the layout glossary tool and dynasty popouts on the item sheet', async () => {
    ...
    const { app, host } = await mountSite('#/item/75bb1422-0084-5c89-9bec-838982478b11')
  ...
  it('renders the partners list on the composed partner-list view', async () => {
    ...
    expect(host.textContent).toContain('National Tile Museum')
    // The object count is this website's own copy over the shared entry, not
    // a generic count.
    expect(host.textContent).toContain('4 object(s) in this site')
    app.unmount()
  }, 60000)

  it('renders a partner profile on the composed record view', async () => {
    const { app, host } = await mountSite('#/partner/dbb0a6e3-2c73-51cc-8f47-ebf68dde4d73')
    ...
    await vi.waitFor(() => expect(host.textContent).toContain('National Tile Museum'), { timeout: 20000 })
    expect(host.textContent).toContain("Lisbon")
  ...
  it("renders a partner's objects on the composed grid results view", async () => {
    const { app, host } = await mountSite('#/partner/dbb0a6e3-2c73-51cc-8f47-ebf68dde4d73/objects')
    await vi.waitFor(() => expect(host.querySelectorAll('.mwnf-grid__tile').length).toBe(4), { timeout: 20000 })
    expect(host.textContent).toContain('National Tile Museum')
    app.unmount()
  }, 60000)
  ...
  it('renders the shared texts and its own over them', async () => {
    ...
    expect(checkTextsRendered(host, {
      namespaces: ['ceramics', 'core', 'layout', 'catalogue', 'record', 'sheet', 'timeline', 'partner', 'gallery'],
    })).toEqual([])
