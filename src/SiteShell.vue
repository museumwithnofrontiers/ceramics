...
<template>
  <SiteShell
    v-bind="$attrs"
    header-home="#/"
    :footer-text="t('ceramics.identity.copyright')"
  >
    <template #brand>
      <span class="site-logo">
        <span class="site-logo-org">{{ t('ceramics.identity.organisation') }}</span>
        <span class="site-logo-title">{{ t('ceramics.identity.title') }}</span>
      </span>
    </template>
    <slot />
  </SiteShell>
